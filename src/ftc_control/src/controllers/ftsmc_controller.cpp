...
FTSMCController::FTSMCController() {
    reset();
}

void FTSMCController::reset() {
    e_nu_old_.setZero();
    nu_c_old_.setZero();
}

double FTSMCController::sig(double x, double beta) const {
    if (std::abs(x) < 1e-3) return 0.0;
    return std::pow(std::abs(x), beta) * (x > 0 ? 1.0 : -1.0);
}

Eigen::Matrix<double, 6, 6> FTSMCController::getRotationMatrix(const Eigen::Matrix<double, 6, 1>& eta) const {
...
Eigen::Matrix<double, 6, 1> FTSMCController::compute_ftsmc(
    ...
    double dt)
{
    ...
    Eigen::Matrix<double, 6, 1> alpha_sig_e_eta;
    for(int i=0; i<6; ++i) {
        alpha_sig_e_eta(i) = alpha_diag(i) * sig(e_eta(i), beta_diag(i));
    }

    Eigen::Matrix<double, 6, 6> J = getRotationMatrix(eta);
    // nu_c = J^-1 * (dot_eta_d + alpha * sig(e_eta))
    Eigen::Matrix<double, 6, 1> nu_c = J.inverse() * (dot_eta_d + alpha_sig_e_eta);
    Eigen::Matrix<double, 6, 1> dot_nu_c = (nu_c - nu_c_old_) / dt;
    nu_c_old_ = nu_c;

    Eigen::Matrix<double, 6, 1> e_nu = nu_c - nu;

    // PD Sliding Surface: s = e_nu_dot + k1*sig(e_nu) + k2*e_nu
    Eigen::Matrix<double, 6, 1> e_nu_dot = (e_nu - e_nu_old_) / dt;
    e_nu_old_ = e_nu;

    Eigen::Matrix<double, 6, 1> dynamic_term;
    for(int i=0; i<6; ++i) {
        dynamic_term(i) = k1_diag(i) * sig(e_nu(i), gamma_diag(i)) + k2_diag(i) * e_nu(i);
    }

    Eigen::Matrix<double, 6, 1> s_nu = e_nu_dot + dynamic_term;
    ...
    Eigen::Matrix<double, 6, 1> tau_eq = (C * nu) + (D * nu) + g + M * (dot_nu_c + dynamic_term);

    Eigen::Matrix<double, 6, 1> tau_sw_components;
    for(int i=0; i<6; ++i) {
        tau_sw_components(i) = w1_diag(i) * sig(s_nu(i), rho_diag(i)) + w2_diag(i) * s_nu(i);
    }
    Eigen::Matrix<double, 6, 1> tau_sw = M * tau_sw_components;

    return applyLimits(tau_eq + tau_sw);
}

Eigen::Matrix<double, 6, 1> FTSMCController::applyLimits(const Eigen::Matrix<double, 6, 1>& tau) const {
